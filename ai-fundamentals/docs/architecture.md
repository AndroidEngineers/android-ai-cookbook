...
Navigation 3 saves routes; one activity-scoped ViewModel owns the shared draft across Create and Editor. This scope is intentional for the short multi-screen creation flow. Study queues and positions use saveable Compose state. A bounds-checked current-card snapshot is computed before layout to avoid reading past the last card. Favorites, next-review day and last-reviewed day are persisted with each card. Ratings advance only after storage succeeds. Got it schedules tomorrow; Again remains due today. This is a simple daily schedule, not an adaptive spaced-repetition algorithm. The active filter queue stays stable until switching tabs or restarting. On process recreation, saved decks reload from storage; unsaved drafts are intentionally not persisted and the editor explains the empty-draft case.
